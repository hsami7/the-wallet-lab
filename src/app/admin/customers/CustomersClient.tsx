...
export function CustomersClient({ initialCustomers }: { initialCustomers: Customer[] }) {
  ...
  useEffect(() => {
    const results = initialCustomers.map(c => ({
      id: c.id,
      title: c.name,
      subtitle: c.email,
  ...
  }, [initialCustomers, setPageResults]);
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Customers", value: initialCustomers.length, icon: "group" },
          { label: "Active", value: initialCustomers.filter((c) => c.status === "Active").length, icon: "person_check" },
          { label: "New This Month", value: initialCustomers.filter((c) => c.status === "New").length, icon: "person_add" },
          { label: "Avg. Orders", value: (initialCustomers.reduce((acc, c) => acc + c.orders, 0) / (initialCustomers.length || 1)).toFixed(1), icon: "shopping_bag" },
        ].map((s) => (
            ...
            <tbody>
              {filtered.map((c, i) => (
                <tr key={c.id} className={`${i < filtered.length - 1 ? "border-b border-slate-100 dark:border-slate-800" : ""} hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors`}>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statusColors[c.status] || statusColors["Active"]}`}>{c.status}</span>
                  </td>
                  <td className="px-6 py-4">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400 italic">No customers match your criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
